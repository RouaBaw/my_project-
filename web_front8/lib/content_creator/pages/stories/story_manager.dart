...
class _StoryManagerPageState extends State<StoryManagerPage> {
  ...
  Future<void> _submitStoryForReview(int id) async {
    setState(() => _isDeleting = true);
    try {
      final token = GetStorage().read('token');
      final response = await http.post(
        Uri.parse('$baseUrl/stories/$id/submit'),
        headers: {
          'Authorization': 'Bearer $token',
          'Accept': 'application/json',
        },
  ...
  Widget _buildStoryCard(dynamic story, int index, ColorScheme cs) {
    ...
    final status = (story['status'] ?? 'draft').toString();
          ...
          children: [
            ...
            Column(
              children: [
                if (status == 'draft')
                  IconButton(
                    onPressed: _isDeleting ? null : () => _submitStoryForReview(story['id']),
                    tooltip: 'إرسال للمراجعة',
                    icon: Icon(
                      Icons.send_outlined,
                      color: _isDeleting ? Colors.grey : const Color(0xFF2563EB),
                    ),
                  ),
                IconButton(
