...
class QuizController extends Controller
{
    public function submitQuiz(Request $request)
    {
        ...
        return DB::transaction(function () use ($userId, $contentId, $userSubmissions, $totalQuestionsCount) {
            ...
            if ($child && $child->isChild()) {
                PointsService::awardForQuizAttempt($child, $attempt->id, $correctAnswersCount);
                $learningContent = LearningContent::find($contentId);
                NotificationService::childFinishedActivity($child, 'quiz', $learningContent ?? (object) ['id' => $contentId, 'title' => ''], [
    ...
    public function getChildResults($childId)
    {
        ...
        $games = GameResult::with('game:id,title,type')
            ->where('user_id', $childId)
            ->latest()
            ->get();
        $stories = StoryAttempt::with('story:id,learning_content_id,title', 'story.learningContent:id,title,course_name')
            ->where('user_id', $childId)
            ->latest()
            ->get();
