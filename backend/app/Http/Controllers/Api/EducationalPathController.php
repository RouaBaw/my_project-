...
class EducationalPathController extends Controller
{
    ...
    {
        $paths = EducationalPath::with(['creator:id,first_name,last_name', 'contents.packages', 'contents.questions.answers', 'contents.games'])->withCount('contents')->get();
    ...
    public function publishedPaths()
    {
        ...
        $paths = EducationalPath::where(['status' => 'published'])->with([
            'creator:id,first_name,last_name',
            'contents',
            'contents.packages',
            'contents.questions.answers',
            'contents.games' => function ($query) use ($user) {
                if ($user && ($user->isChild() || $user->isParent())) {
                    $query->where('status', 'published');
                }
            }
        ])->withCount([
            'contents',
            'contents as games_count' => function ($query) {
    ...
    public function store(Request $request)
    {
        ...
        NotificationService::pathSubmittedForReview($educationalPath, $educationalPath->creator_id);
    ...
    public function reviewPath(Request $request, $id)
    {
        ...
        NotificationService::contentReviewed(
            $path->creator_id,
            'path',
            $path,
            $request->status,
