...
class LearningContentController extends Controller
{
    ...
    {
        $user = auth()->user();
        $userId = $user?->id; // جلب معرف المستخدم الحالي
        $isParentUser = $user && $user->isParent();
        $isChildUser = $user && $user->isChild();

        if ($user && $user->isChild() && !$user->hasActiveSubscription()) {
            return response()->json([
                'message' => 'يلزم وجود اشتراك فعّال لهذا الطفل لعرض تفاصيل المحتوى',
        ...
        $path = \App\Models\EducationalPath::with([
            ...
            'contents.packages' => function ($query) use ($isParentUser) {
                if ($isParentUser) {
                    $query->whereRaw('1 = 0');
                }
            },
            'contents.stories' => function ($query) use ($user, $isParentUser) {
                ...
                $query->with(['contents', 'questions.answers']);
            },
            'contents.games' => function ($query) use ($isParentUser, $isChildUser) {
                if ($isParentUser) {
                    $query->whereRaw('1 = 0');
                    return;
                }
            ...
            'contents.questions' => function ($query) use ($isParentUser) {
                if ($isParentUser) {
                    $query->whereRaw('1 = 0');
                }
            },
            ...
            'contents.ratings' => function ($query) use ($userId, $isParentUser) {
                if ($isParentUser) {
                    $query->whereRaw('1 = 0');
                    return;
                }
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            ...
            NotificationService::courseSubmittedForReview($content, auth()->id());
