...
class ContentPackageController extends Controller
{
    ...
    public function courseGames($id)
    {
        // نقوم بجلب المحتوى التعليمي مع تحميل الأسئلة والإجابات المرتبطة بكل سؤال
        $content = LearningContent::with('games')->findOrFail($id);
    ...
    {
        $package = Game::findOrFail($id);
        $package->delete();

        return response()->json(['message' => 'تم حذف العبة من المحتوى']);
    }
