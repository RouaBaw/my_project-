...
import 'package:untitled1/screens/stories/story_list_page.dart';
...
class _PlaylistState extends State<Playlist> {
  ...
  Future<void> _fetchPathDetails() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final decoded = jsonDecode(response.body);
        setState(() {
          requiresSubscription = decoded['requires_subscription'] == true;
          subscriptionMessage = decoded['message']?.toString();
          isLoading = false;
        });
      } else {
    ...
    return Scaffold(
      ...
      body: isLoading
        ...
        itemBuilder: (context, index) {
          ...
          final int gameCount = (content['games'] as List).length;
          final int storyCount = (content['stories'] as List? ?? []).length;
                    ...
                    Positioned(
                      ...
                      child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                          decoration: BoxDecoration(color: Colors.blue.withOpacity(0.9),
                              borderRadius: BorderRadius.circular(5)), child: Text('فيديوهات $videoCount | ألعاب $gameCount | قصص $storyCount', style: const TextStyle(color: Colors.white, fontSize: 11))),
                    ),
                  ],
  ...
  Widget _buildFooter(dynamic content) {
    final List stories = content['stories'] ?? [];
    final List packages = content['packages'] ?? [];
    return Row(
      children: [
        _buildTag(Icons.help_outline, "${(content['questions'] as List).length} أسئلة", Colors.green),
        const SizedBox(width: 8),
        _buildTag(Icons.auto_stories_outlined, "${stories.length} قصص", Colors.blue),
        const Spacer(),
        if (stories.isNotEmpty)
          TextButton(
            onPressed: () => Get.to(() => StoryListPage(
                  contentId: content['id'],
                  contentTitle: content['title'],
                )),
            child: Row(
              children: const [
                Text("القصص", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        TextButton(
          onPressed: () {
            if (packages.isEmpty && stories.isNotEmpty) {
              Get.to(() => StoryListPage(
                    contentId: content['id'],
                    contentTitle: content['title'],
                  ));
              return;
  ...
  Widget _buildSubscriptionLockedState() {
    return Center(
      child: Padding(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.lock_outline_rounded, size: 42, color: Color(0xFF4F46E5)),
              ),
              const SizedBox(height: 16),
              const Text(
                'يلزم وجود اشتراك فعّال',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 10),
              Text(
                subscriptionMessage ?? 'هذا القسم متاح فقط للأطفال الذين لديهم اشتراك فعّال.',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.grey.shade700, height: 1.6),
              ),
              const SizedBox(height: 18),
              const Text(
                'يمكن لولي الأمر تفعيل الاشتراك من قسم إدارة الأبناء داخل التطبيق.',
                textAlign: TextAlign.center,
